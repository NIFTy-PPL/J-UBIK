...
def _process_frequency(
    ...
    """Describe the spectral axis in the header and, if needed, in a table.
    ...
    Uniform channels get a plain linear axis (``CTYPEn = FREQ``), which is exact
    and readable by every tool. Non-uniform channels (e.g. two disjoint spectral
    windows with a gap) cannot be described by a single ``CRVALn``/``CDELTn``, so
    a FITS ``-TAB`` lookup-table axis is written instead and the accompanying
    coordinate-array HDU is returned for attachment to the HDUList.
    ...
    -------
    tuple[Optional[fits.BinTableHDU], NDArray]
        The ``WCS-FREQ`` coordinate-array table (`None` if the axis was squeezed
        or uniform) and the possibly squeezed field.
    """
