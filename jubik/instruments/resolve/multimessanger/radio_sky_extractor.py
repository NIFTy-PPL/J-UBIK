...
def build_radiofy_sky(sky_domain_shape: tuple[int]):
    ...
    `docs/source/user/canonical-sky-design.md`). The radio response owns the conversion to the
...
class RadioSkyExtractor(jft.Model):
    ...
    (`dim0 = +Dec`/North, `dim1 = -RA`/West; see `docs/source/user/canonical-sky-design.md`). The
