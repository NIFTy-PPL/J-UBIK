...
POSITION_ANGLE_DEFAULT = 0.*u.deg
YAML_POSITION_ANGLE_KEY = 'position_angle'


def _get_position_angle(grid_config: dict) -> u.Quantity:
    """Get the astronomical position angle of the reconstruction grid.
    ...
    ----------
    grid_config : dict
        Configuration which may hold `position_angle` as an angle carrying an
        angular unit, e.g. `12deg`. (default `0deg`)

    Returns
    -------
    u.Quantity
        The position angle of the grid.
    """
...
class WcsModel:
    ...
    def from_yaml_dict(cls, grid_config: dict):
        ...
        position_angle: str
            ...
            See also `_get_position_angle`. (default `0.0deg`)
        frame: str
            See also `CoordinatesSystemModel`. (default `icrs`)
        '''

        position_angle = _get_position_angle(grid_config)
