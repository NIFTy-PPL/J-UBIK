...
def create_mock_data(
    tel_info, file_info, grid_info, prior_info, plot_info, seed, response_dict
):
    ...
    if "kernel" in response_dict:
        masked_mock_data = response_dict["R"](
            sky(mock_sky_position), response_dict["kernel"]
        )
    else:
    ...
    if plot_info["enabled"]:
        ...
        mock_output = create_output_directory(join(file_info["res_dir"], "mock_setup"))
        ...
        for key, sky_comp in sky_comps.items():
            plot_rgb(
                ...
                name=join(mock_output, f"mock_rgb_log_{key}"),
                log=True,
            )
            plot_rgb(
                sky_comp(mock_sky_position),
                ...
                sat_max=(
                    0.1 * np.max(sky_comp(mock_sky_position), axis=(1, 2))
                ).tolist(),
            )
            plot_result(
                sky_comp(mock_sky_position),
