...
def _spectral_bins_within_range(grid: Grid, srange: tuple[float]) -> tuple[int]:
    sbins = []
    for ii, cr in enumerate(grid.spectral.color_ranges):
        if cr.start in srange and cr.end in srange:
            ...
            raise ValueError(
                "Please set up your spectrum such that the bins "
...
def spectral_slices(
    grid: Grid, spectral_ranges: list[ColorRange]
) -> list[tuple[int | None]]:
    """
    ...
    lsbs = [
        last_spectral_bin(grid, spectral_range) + 1
