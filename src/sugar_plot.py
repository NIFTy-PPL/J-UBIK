...
def plot_pspec(
    ...
    results_path = create_output_directory(
        join(output_directory, f"{directory_prefix}pspec")
    )
...
def plot_sample_and_stats(
    ...
    for key in operators_dict:
        ...
        samples_result_path = create_output_directory(
            join(results_path, "samples", f"iteration_{iteration}")
        )
        if e_length == 3:
            rgb_result_path_samples = create_output_directory(
                join(results_path, "rgb", "samples", f"iteration_{iteration}")
            )
        ...
        if plot_samples:
            for i in range(n_samples):
                filename_samples = join(
                    samples_result_path, f"sample_{i + 1}_it_{iteration}.png"
                )
                ...
                if e_length == 3 and plot_rgb_samples:
                    # Plot RGB
                    rgb_filename = join(
                        rgb_result_path_samples, f"sample_{i + 1}_{iteration}_rgb"
                    )
                    plot_rgb(
                        operator_samples[i],
                        sat_min=rgb_min_sat,
        ...
        if len(sample_list) > 1:
            ...
            if e_length == 3:
                ...
                plot_rgb(mean, sat_min=rgb_min_sat, sat_max=rgb_max_sat, name=rgb_name)
                plot_rgb(
                    mean,
                    sat_min=rgb_min_sat,
                    sat_max=None,
...
def plot_erosita_priors(
    ...
    for i, pos in enumerate(positions):
        ...
        for key, val in plottable_samples.items():
            plot_result(
                val(pos),
                output_file=filename_base.format(key),
                logscale=log_scale,
                adjust_figsize=adjust_figsize,
                title=[
                    f"E_min={emin}, E_max={emax}" for emin, emax in zip(e_min, e_max)
                ],
    ...
    if signal_response:
        ...
        mask_adj = linear_transpose(
            response_dict["mask"], np.zeros((n_modules, epix, spix, spix))
        )
        # TODO: unify cases
        if "kernel" in response_dict:
            R = lambda x: mask_adj(response_dict["R"](x, response_dict["kernel"]))[0]
        else:
        ...
        for i, pos in enumerate(positions):
            for key, val in plottable_samples.items():
                ...
                for id, samps in enumerate(tmp):
                    ...
                    plot_result(
                        samps,
                        output_file=filename.format(key),
                        logscale=log_scale,
                        title=[
                            f"E_min={emin}, E_max={emax}"
                            for emin, emax in zip(e_min, e_max)
                        ],
                        common_colorbar=common_colorbar,
                        adjust_figsize=adjust_figsize,
                    )
...
def plot_uncertainty_weighted_residuals(
    samples,
    operator_dict,
    diagnostics_path,
    response_dict,
    ...
    for key, op in operator_dict.items():
        ...
        plot_result(
            masked_uwrs,
            output_file=join(diagnostics_path, f"{base_filename}{key}.png"),
            **plot_kwargs,
        )
        if n_bins:
            if range is None:
            ...
            hist, edges = np.histogram(
                masked_uwrs.reshape(-1), bins=n_bins, range=range
            )
            title = plot_kwargs["title"] if plot_kwargs is not None else None
            plot_histograms(
                hist,
                edges,
                join(diagnostics_path, f"{base_filename}{key}_hist.png"),
                title=title,
            )
    return res_dict


def plot_noise_weighted_residuals(
    samples,
    operator_dict,
    diagnostics_path,
    response_dict,
    ...
    for key, op in operator_dict.items():
        ...
        for id, i in enumerate(masked_nwrs):
            ...
            plot_result(
                np.mean(i, axis=0),
                output_file=join(
                    results_path, f"{base_filename}{key}_tm{id + 1}_mean.png"
                ),
                **plot_kwargs,
            )

            if n_bins is not None:
                hist_func = lambda x: jnp.histogram(
                    x.reshape(-1), bins=n_bins, range=extent
                )[0]
                edges_func = lambda x: jnp.histogram(
                    x.reshape(-1), bins=n_bins, range=extent
                )[1]
                ...
                hist_filename = join(
                    results_path, f"{base_filename}{key}_tm{id + 1}_hist.png"
                )
                plot_histograms(
                    mean_hist,
                    edges,
...
def plot_2d_gt_vs_rec_histogram(
    ...
    if "kernel" in response_dict:
        ...
        Rs_sample_dict = {
            key: [R(op(s)) for s in samples] for key, op in operator_dict.items()
        }
