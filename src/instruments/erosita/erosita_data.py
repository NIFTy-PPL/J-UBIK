...
from ...utils import save_to_pickle, get_config, create_output_directory, copy_config


def create_erosita_data_from_config(config, response_dict):
...
def mask_erosita_data_from_disk(file_info, tel_info, grid_info, mask_func):
    ...
    for tm_id in tel_info["tm_ids"]:
        ...
        for output_filename in output_filenames:
            data.append(
                fits.open(join(file_info["obs_path"], "processed", output_filename))[
                    0
                ].data
            )
    ...
    save_to_pickle(
        masked_data_vector.tree, join(file_info["res_dir"], file_info["data_dict"])
    )
    return masked_data_vector


def generate_erosita_data_from_config(config):
    """
    ...
    processed_obs_path = create_output_directory(
        join(obs_path, file_info["processed_obs_folder"])
    )
    for tm_id in tel_info["tm_ids"]:
        output_filenames = f"tm{tm_id}_" + file_info["output"]
