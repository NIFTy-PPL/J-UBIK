...
class ChandraObservationInformation:
    ...
    def __init__(
        ...
        ###########################################################################
        self.obsInfo["ra"] = float(
            rt.dmkeypar(infile=self.obsInfo["event_file"], keyword="RA_NOM", echo=True)
        )
        self.obsInfo["dec"] = float(
            rt.dmkeypar(infile=self.obsInfo["event_file"], keyword="DEC_NOM", echo=True)
        )
        self.obsInfo["roll"] = float(
            rt.dmkeypar(
                infile=self.obsInfo["event_file"], keyword="ROLL_NOM", echo=True
            )
        )
        self.obsInfo["duration"] = float(
            rt.dmkeypar(
                infile=self.obsInfo["event_file"], keyword="EXPOSURE", echo=True
            )
        )
    ...
    def get_data(self, outfile):
        ...
        infile += "[x={0:.6f}:{1:.6f},".format(
            self.obsInfo["x_min"], self.obsInfo["x_max"]
        )
        infile += "y={0:.6f}:{1:.6f},".format(
            self.obsInfo["y_min"], self.obsInfo["y_max"]
        )
        infile += "energy={0:.4f}:{1:.4f}]".format(
            1.0e3 * self.obsInfo["energy_min"], 1.0e3 * self.obsInfo["energy_max"]
        )
        ...
        evts = np.array([evts["x"], evts["y"], np.log(1.0e-3 * evts["energy"])])
        evts = evts.transpose()
        if self.obsInfo["energy_ranges"]:
            bins = (
                self.obsInfo["npix_s"],
                self.obsInfo["npix_s"],
                np.log(self.obsInfo["energy_ranges"]),
            )
        else:
            bins = (
                self.obsInfo["npix_s"],
        ...
        data, edges = np.histogramdd(
            evts, bins=bins, range=ranges, density=False, weights=None
        )
    ...
    def get_exposure(self, outroot, res_xy=0.5, energy_subbins=10):
        ...
        ##############################
        det_str = rt.dmkeypar(
            infile=self.obsInfo["event_file"], keyword="detnam", echo=True
        )
        ...
        for chip in range(0, len(chips_on)):
            ...
            xcrit = (
                (self.obsInfo["x_min"] <= min(edgex) <= self.obsInfo["x_max"])
                or (self.obsInfo["x_min"] <= max(edgex) <= self.obsInfo["x_max"])
                or (
                    min(edgex) < self.obsInfo["x_min"]
                    and max(edgex) > self.obsInfo["x_max"]
                )
            )
            ycrit = (
                (self.obsInfo["y_min"] <= min(edgey) <= self.obsInfo["y_max"])
                or (self.obsInfo["y_min"] <= max(edgey) <= self.obsInfo["y_max"])
                or (
                    min(edgey) < self.obsInfo["y_min"]
                    and max(edgey) > self.obsInfo["y_max"]
                )
            )
        ...
        for det in det_num:
            ...
            rt.asphist(
                infile=self.obsInfo["aspect_sol"],
                outfile=outf,
                evtfile=evtf,
                res_xy=self.obsInfo["asphist_res_xy"],
                clobber="yes",
            )
        ...
        logstep = (
            np.log(self.obsInfo["energy_max"] / self.obsInfo["energy_min"])
            / self.obsInfo["npix_e"]
        )
        ...
        for i in range(0, self.obsInfo["npix_e"]):
            ...
            if self.obsInfo["energy_ranges"]:
                ...
                src_e_max = np.round(np.exp(logemin + (i + 1.0) * logstep), decimals=10)
            ...
            # for more than one energy sub-bin, request a flat spectrum over the channel
                ...
                bins = np.linspace(
                    src_e_min,
                    src_e_max,
                    self.obsInfo["exp_ebins_per_bin"],
                    endpoint=True,
                )
                ...
                with open(specfile, "w+") as sf:
                    for energy in bins:
                        sf.write(
                            "{:.9f}\t{:9f}\n".format(
                                energy, 1.0 / self.obsInfo["exp_ebins_per_bin"]
                            )
                        )
                energy = 1.0
            ...
            for det in det_num:
                ...
                rt.mkinstmap(
                    ...
                    monoenergy=energy,
                    pixelgrid=pgrid,
                    obsfile=self.obsInfo["event_file"],
                    detsubsys=subdet,
                    maskfile=self.obsInfo["mask_file"],
                    clobber="yes",
            ...
            for det in det_num:
                ...
                expmap_dic[det] = outf
                rt.mkexpmap(
                    instmapfile=instmap_dic[det],
                    outfile=outf,
                    asphistfile=asphist_dic[det],
                    xygrid=xygrid,
                    clobber="yes",
                    normalize="no",
                )
            ...
            rt.dmimgcalc(
                infile=ifile, infile2="none", out=outfs, operation=opstr, clobber="yes"
            )
        ...
        ####################################################
        expmap = np.zeros(
            [self.obsInfo["npix_s"], self.obsInfo["npix_s"], self.obsInfo["npix_e"]]
        )
    ...
    def get_psf_fromsim(
        self, location, outroot, num_rays=1e4, detector_type=None, aspect_blur=None
    ):
        """
        ...
        # 2.c) detector offset
        marx_file = (
            os.environ["MARX_DATA_DIR"]
            + "/caldb/telD1999-07-23aimptsN0002.fits"
            + "[AIMPOINTS][AIMPOINT_NAME="
            + det_short
            + "][cols AIMPOINT]"
        )
        ...
        detoffset_x = float(
            rt.dmkeypar(infile=self.obsInfo["event_file"], keyword="SIM_X", echo=True)
        ) - float(marx_nom[1])
        detoffset_z = float(
            rt.dmkeypar(infile=self.obsInfo["event_file"], keyword="SIM_Z", echo=True)
        ) - float(marx_nom[3])
        ...
        pointing_ra = rt.dmkeypar(
            infile=self.obsInfo["event_file"], keyword="RA_NOM", echo=True
        )
        pointing_dec = rt.dmkeypar(
            infile=self.obsInfo["event_file"], keyword="DEC_NOM", echo=True
        )
        pointing_roll = rt.dmkeypar(
            infile=self.obsInfo["event_file"], keyword="ROLL_NOM", echo=True
        )

        # 2.c) Timing Information
        tstart = rt.dmkeypar(
            infile=self.obsInfo["event_file"], keyword="TSTART", echo=True
        )
        ...
        for i in range(0, self.obsInfo["npix_e"]):
            ...
            src_e_max = np.exp(logemin + (i + 1.0) * logstep)
            ...
            infile = outfits + "[EVENTS][x={0:.1f}:{1:.1f}, y={2:.1f}:{3:.1f}]".format(
                self.obsInfo["x_min"],
                self.obsInfo["x_max"],
                self.obsInfo["y_min"],
                self.obsInfo["y_max"],
            )
            infile += "[bin x={:.1f}:{:.1f}:#{:d}, y={:.1f}:{:.1f}:#{:d}]".format(
                self.obsInfo["x_min"],
                self.obsInfo["x_max"],
                self.obsInfo["npix_s"],
                self.obsInfo["y_min"],
