...
def build_plot_sky_residuals(
    ...
    def sky_residuals(
        ...
        small_mean, small_std = get_position_or_samples_of_model(
            position_or_samples, small_sky_model
        )
        ...
        for kk, (jj, filter_name) in zip(
            range(ii + 1, len(axes)), enumerate(sky_model_with_key.target.keys())
        ):
        ...
        if overwrite_model is not None:
            ...
            mean, _ = get_position_or_samples_of_model(position_or_samples, model)
...
def build_color_components_plotting(
    ...
    def color_plot(
        ...
        for ax, cor, comp, mix in zip(
            ...
            im0 = ax[0].imshow(
                cor, origin="lower", norm=LogNorm(), interpolation="None"
            )
...
def get_alpha_nonpar(lens_system):
    ...
    if isinstance(llm, jft.CorrelatedMultiFrequencySky):
        ...
        if llm.spectral_index_distribution is not None:
            ll_spectral_index = jft.Model(
                lambda x: llm.spectral_index_distribution(x)[
                    : ll_shape[0], : ll_shape[1]
                ],
                domain=llm.domain,
            )
        else:

            def ll_spectral_index(_):
                return np.zeros((12, 12))

        ll_nonparametric = jft.Model(
            lambda x: llm.reference_frequency_correlated_field(x)[
                : ll_shape[0], : ll_shape[1]
            ],
            domain=llm.domain,
        )
    ...
    if isinstance(slm, jft.CorrelatedMultiFrequencySky):
        ...
        if sl_spectral_index is None:
            ...
            def sl_spectral_index(_):
                return np.zeros((12, 12))

        if sl_nonparametric is None:

            def sl_nonparametric(_):
                return np.zeros((12, 12))

    elif isinstance(slm, ColorMix):
        sl_spectral_index = slm.color_matrix
        sl_nonparametric = jft.Model(lambda x: slm.components(x)[0], domain=slm.domain)
    else:
    ...
    return (ll_spectral_index, ll_nonparametric, sl_spectral_index, sl_nonparametric)
...
def build_get_values(
    ...
    if parametric:
        ...
        def convergence_nonpar_model(_):
            return np.zeros((12, 12))

        sky_model = lens_system.get_forward_model_parametric()
        lensed_light_model = lens_system.get_forward_model_parametric(only_source=True)

    else:
        convergence_model = lens_system.lens_plane_model.convergence_model
        convergence_nonpar_model = (
...
def build_plot_source(
    ...
    norm_source_parametric = plotting_config.get("norm_source_parametric", Normalize)
    ...
    def plot_source(
        ...
        if isinstance(position_or_samples, jft.Samples):
            ...
            slnonpar = jft.mean([slight_nonparametric(x) for x in position_or_samples])
            slpar = jft.mean([slight_parametric(x) for x in position_or_samples])
        ...
        source_light_min = (
            next(iter(source_light.values())).min()
            if isinstance(source_light, dict)
            else source_light.min()
        )
        source_light_max = (
            next(iter(source_light.values())).max()
            if isinstance(source_light, dict)
            else source_light.max()
        )
        ...
        axes = axes.flatten()
        ims = ims.flatten()
        for ii, (energy_range, fld) in enumerate(
            zip(grid.spectral.color_ranges, source_light)
        ):
        ...
        if state_or_none is not None:
            fig.tight_layout()
            fig.savefig(
                join(lens_dir, f"{attach_name}{state_or_none.nit:02d}.png"), dpi=300
            )
            plt.close()
        else:
            plt.show()
...
def build_plot_lens_system(
    ...
    share_source = plotting_config.get("share_source", False)
    ...
    def plot_lens_system(
        ...
        if isinstance(position_or_samples, jft.Samples):
            (
                ...
                convergence_nonpar,
                sky,
            ) = jft.mean([get_values(si) for si in position_or_samples])
            ...
            (
                ...
                convergence_nonpar,
                sky,
            ) = get_values(position_or_samples)
        ...
        ims[0, 0] = axes[0, 0].imshow(lla, origin="lower", interpolation="None")
        ims[0, 1] = axes[0, 1].imshow(lln, origin="lower", interpolation="None")
        ...
        vmin_source = np.max((min_source, source_light.min()))
        vmax_source = source_light.max()

        vmin_llight = np.max((min_source, lensed_light.min())) if share_source else None
        vmax_llight = lensed_light.max() if share_source else None

        vmin_lens = np.max((min_lens, lens_light.min()))
        vmax_lens = lens_light.max()
        ...
        for ii, energy_range in enumerate(grid.spectral.color_ranges):
            ...
            ims[0, ii + light_offset] = axes[0, ii + light_offset].imshow(
                ...
                origin="lower",
                extent=lens_ext,
                norm=norm_lens(vmin=vmin_lens, vmax=vmax_lens),
                interpolation="None",
            )
            ...
            ims[1, ii + light_offset] = axes[1, ii + light_offset].imshow(
                ...
                origin="lower",
                extent=source_ext,
                norm=norm_source(vmin=vmin_source, vmax=vmax_source),
                interpolation="None",
            )
            ...
            ims[2, ii + light_offset] = axes[2, ii + light_offset].imshow(
                ...
                origin="lower",
                extent=lens_ext,
                norm=norm_source(vmin=vmin_llight, vmax=vmax_llight),
                interpolation="None",
            )
