...
class AdditiveStdValueBuilder(InverseStdBuilder):
    ...
    def build(self, std: np.ndarray, mask: np.ndarray) -> jft.Model:
        ...
        if self.shape_type == StdValueShapeType.filter:
            ...
            def apply(x):
                return 1 / (std[mask] + distribution(x))
            ...
            std = std[mask]
            std = [std[sh[ii] : sh[ii + 1]] for ii in range(len(sh) - 1)]

            def apply(x):
                val = distribution(x)
                std_new = [std[ii] + val[ii] for ii in range(len(val))]
                return 1 / reduce(jnp.append, std_new)
...
def build_inverse_standard_deviation(
    filter_name: str,
    config: AdditiveStdValueConfig | MultiplicativeStdValueConfig | None,
) -> AdditiveStdValueBuilder | None:
    if config is None:
        return None
    ...
    if isinstance(config, AdditiveStdValueConfig):
        return AdditiveStdValueBuilder(
            filter=filter_name,
            distribution=config.distribution,
            shape_type=config.shape_type,
        )
