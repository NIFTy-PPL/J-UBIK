...
priors_diffuse = {
    ...
    "fluctuations": (2.0, 0.5),  # 1.0, 1e-2
    # Exponent of power law power spectrum component
    "loglogavgslope": (-2.5, 1.0),  # -6.0, 1
    # Amplitude of integrated Wiener process power spectrum component
    "flexibility": (2.0, 0.5),  # 2.0, 1.0
...
signal_response = mask @ normed_exposure @ conv  # zp.adjoint @ diffuse

ic_newton = ift.AbsDeltaEnergyController(
    name="Newton", deltaE=0.5, iteration_limit=15, convergence_level=3
)
...
minimizer_sampling = ift.NewtonCG(
    ift.AbsDeltaEnergyController(
        name="Sampling (nonlin)", deltaE=0.5, convergence_level=2, iteration_limit=10
    )
)
...
if True:
    ...
    for ii in range(10):
        KL = ift.GeoMetricKL(
            ...
            minimizer_sampling,
            True,
            constants=psf_pos.keys(),
            point_estimates=psf_pos.keys(),
            comm=mpi.comm,
        )
        KL, _ = minimizer(KL)
