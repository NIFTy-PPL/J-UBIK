...
def optimize_kl(likelihood_energy,
    ...
    for iglobal in range(initial_index, total_iterations):
        ...
        if not comm(iglobal) is None:
            try:
                ...
                pass
    ift.myassert(_number_of_arguments(inspect_callback) in [1, 2, 3])
    ift.myassert(_number_of_arguments(terminate_callback) == 1)
    mf_dom = isinstance(likelihood_energy(initial_index).domain,ift.MultiDomain)
    if mf_dom:
        dom =ift.MultiDomain.union([likelihood_energy(iglobal).domain
                                 for iglobal in
    ...
    if plot_latent:
        ...
        plottable_operators["latent"] = ift.ScalingOperator(dom, 1.)

    # Initial position
    mean = initial_position
    ift.check_MPI_synced_random_state(comm(initial_index))
    if mean is None:
        mean = 0.1 * ift.from_random(dom)
    ift.myassert(dom is mean.domain)
    # /Initial position
    ...
    for iglobal in range(initial_index, total_iterations):
        ham =ift.StandardHamiltonian(likelihood_energy(iglobal), sampling_iteration_controller(iglobal))
        ...
        if output_directory is not None:
            _plot_operators(output_directory, iglobal, plottable_operators, sl,
                            ground_truth_position, comm(iglobal), save_strategy, obs_type)
...
def _plot_operators(output_directory, index, plottable_operators, sample_list,
                    ground_truth, comm, save_strategy, obs_type):
    if not isinstance(plottable_operators, dict):
        raise TypeError
    ...
    for name, op in plottable_operators.items():
        ...
        if ground_truth is None or not _MPI_master(comm):
            ground_truth_sl = ift.Nop()
        else:
            ground_truth_sl = SampleList([ground_truth])
        if h5py and obs_type is None:
...
def _plot_samples(file_name, samples, ground_truth, comm, plotting_kwargs):
    ...
    if _MPI_master(comm):
        ...
        for kk in keys:
            ...
            if ift.plottable2D(samples[0][kk]):
                if ground_truth is not None:
...
def _is_subdomain(sub_domain, total_domain):
    if not isinstance(sub_domain, (MultiDomain, ift.DomainTuple)):
        raise TypeError
