...
def optimize_kl(likelihood_energy,
    ...
    obs_type : string or None
        Describes the observation type. If obs_type is set to None the observation
        is considered as a single frequency observation. Multi-frequency observations
        are only possible if astropy is True.
    Returns
    -------
    sl : SampleList
    ...
    for iglobal in range(initial_index, total_iterations):
        for (obj, cls) in [(likelihood_energy, ift.Operator), (kl_minimizer, ift.DescentMinimizer),
                           (nonlinear_sampling_minimizer, (ift.DescentMinimizer, type(None))),
                           (constants, (list, tuple)), (point_estimates, (list, tuple)),
                           (transitions, (ift.Operator, type(None))), (n_samples, int)]:
        ...
        if sampling_iteration_controller(iglobal) is None:
            ift.myassert(n_samples(iglobal) == 0)
        else:
            ift.myassert(isinstance(sampling_iteration_controller(iglobal), ift.IterationController))
        ift.myassert(likelihood_energy(iglobal).target is ift.DomainTuple.scalar_domain())
        if not comm(iglobal) is None:
            try:
                import mpi4py
                ift.myassert(isinstance(comm(iglobal), mpi4py.MPI.Intracomm))
            except ImportError:
                pass
    ift.myassert(_number_of_arguments(inspect_callback) in [1, 2])
    ift.myassert(_number_of_arguments(terminate_callback) == 1)

    if not likelihood_energy(initial_index).target is ift.DomainTuple.scalar_domain():
        raise TypeError
    # /Sanity check of input

    # Initial position
    ift.check_MPI_synced_random_state(comm(initial_index))
    if initial_position is None:
        mean = ift.full(ift.makeDomain({}), 0.)
    else:
    ...
    def trans(iglobal):
        ...
        if dom is tgt:
            return ift.Operator.identity_operator(dom)
    ...
    for iglobal in range(initial_index, total_iterations):
        ...
        if iglobal == 0:
            ift.myassert(mean.domain is trans(iglobal).domain)
        else:
            ift.myassert(likelihood_energy(iglobal - 1).domain is trans(iglobal).domain)
    # /Automatic transitions
    ...
    if initial_index == 0:
        energy_history = ift.EnergyHistory()

    for iglobal in range(initial_index, total_iterations):
        mean = trans(iglobal)(mean)
        dom = mean.domain
        mf_dom = isinstance(dom, ift.MultiDomain)
        if not mf_dom:
        ...
        count = ift.CountingOperator(lh.domain)
        ham = ift.StandardHamiltonian(lh @ count, sampling_iteration_controller(iglobal))
        ...
        ift.check_MPI_synced_random_state(comm(iglobal))
        ift.check_MPI_equality(likelihood_energy(iglobal).domain, comm(iglobal))
        ift.check_MPI_equality(mean.domain, comm(iglobal))
        ift.check_MPI_equality(mean, comm(iglobal))

        if n_samples(iglobal) == 0:
            e = ift.EnergyAdapter(mean_iter, ham, constants=constants(iglobal),
                              want_metric=_want_metric(minimizer))
            if comm(iglobal) is None:
                e, _ = minimizer(e)
                mean = ift.MultiField.union([mean, e.position]) if mf_dom else e.position
                sl = SampleList([mean])
...
def _plot_operators(index, export_operator_outputs, sample_list, comm, obs_type):
    if not isinstance(export_operator_outputs, dict):
        raise TypeError
    ...
    for name, op in export_operator_outputs.items():
        if not _is_subdomain(op.domain, sample_list.domain):
            continue
        if not astropy and obs_type is not None:
            raise TypeError
        op_direc = join(_output_directory, name)
        ...
        if astropy:
            try:
                ...
                sample_list.save_to_fits(file_name_base, op=op, overwrite=True, **cfg, obs_type = obs_type)
            except ValueError:
                pass
...
def _minisanity(likelihood_energy, iglobal, sl, comm, plot_minisanity_history):
    ...
    s, ms_val = ift.minisanity(likelihood_energy, sl, terminal_colors=False,
                           return_values=True)
    ift.check_MPI_equality(ms_val, comm(iglobal))
...
def _report_to_logger_and_file(report, file_name, iglobal, comm, to_logger,
    ...
    if not only_master:
        report = ift.allreduce_sum([[report]], comm(iglobal))
        report = [f"Task {ii}\n{rr}" for ii, rr in enumerate(report)]
        report = "\n".join(report)

    if _MPI_master(comm(iglobal)):
        if to_logger:
            ift.logger.info(report)
...
def _handle_terminate_callback(terminate_callback, iglobal, comm):
    ...
    ift.check_MPI_equality(terminate, comm(iglobal))
    return terminate


def _MPI_master(comm):
    return ift.get_MPI_params_from_comm(comm)[2]


def _make_callable(obj):
    if callable(obj) and not (isinstance(obj, ift.Minimizer) or isinstance(obj, ift.IterationController) or
                              isinstance(obj, ift.Operator)):
        return obj
    else:
        return lambda x: obj
...
class _InitializeOperator(ift.Operator):
    def __init__(self, domain, target, std):
        self._domain = ift.MultiDomain.make(domain)
        self._target = ift.MultiDomain.make(target)
    ...
    def apply(self, x):
        ...
        diff_domain = ift.makeDomain({kk: self._target[kk] for kk in diff_keys})
        diff_fld = ift.from_random(diff_domain, std=self._std)
        ift.myassert(len(set(diff_fld.keys()) & set(self._domain.keys())) == 0)
