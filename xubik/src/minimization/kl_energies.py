...
def _reduce_field(field, keys):
    if isinstance(field, ift.MultiField) and len(keys)>0:
...
def _reduce_by_keys(field, operator, keys):
    ...
    """
    ift.myassert(ift.is_fieldlike(field))
    ift.myassert(ift.is_operator(operator))
    if isinstance(field, ift.MultiField):
    ...
    ift.myassert(len(keys) == 0)
    return field, operator


class _SelfAdjointOperatorWrapper(ift.EndomorphicOperator):
...
def draw_samples(position, H, minimizer, n_samples, mirror_samples, napprox=0,
    ...
    if not isinstance(mirror_samples, bool):
        raise TypeError
    if not isinstance(H, ift.StandardHamiltonian):
        raise TypeError
    if isinstance(position, ift.MultiField):
    ...
    if geometric:
        ...
        if isinstance(dtype, dict):
            ift.myassert(all([dtype[k] is not None for k in dtype.keys()]))
        else:
            ift.myassert(dtype is not None)
        scale = ift.ScalingOperator(f_lh.target, 1., dtype)
        fl = f_lh(ift.Linearization.make_var(sam_position))

        transformation = ift.ScalingOperator(f_lh.domain, 1.) + fl.jac.adjoint @ f_lh
        ...
        met = ift.SamplingEnabler(ift.SandwichOperator.make(fl.jac, scale),
                              ift.ScalingOperator(fl.domain, 1., float),
                              H.iteration_controller)
    else:
        met = H(ift.Linearization.make_var(sam_position, want_metric=True)).metric
    if napprox >= 1:
        met._approximation = ift.makeOp(ift.approximation2endo(met, napprox))
    # /Construct transformation
    ...
    ift.utilities.check_MPI_equality(sseq, comm)
    y = None

    ntask, rank, _ = ift.get_MPI_params_from_comm(comm)
    for i in range(*ift.shareRange(len(sseq), ntask, rank)):
        with ift.random.Context(sseq[i]):
            ...
            if geometric:
                ...
                en = ift.GaussianEnergy(m) @ transformation
                en = ift.EnergyAdapter(pos, en, nanisinf=True, want_metric=True)
                en, _ = minimizer(en)
...
def SampledKLEnergy(position, hamiltonian, n_samples, minimizer_sampling,
    ...
    if not isinstance(mirror_samples, bool):
        raise TypeError
    if not (minimizer_sampling is None or isinstance(minimizer_sampling, ift.DescentMinimizer)):
        raise TypeError
    if isinstance(position, ift.MultiField):
    ...
    if isinstance(position, ift.MultiField) and len(invariant) > 0:
...
class SampledKLEnergyClass(ift.Energy):
    ...
    def __init__(self, sample_list, hamiltonian, constants, invariants, nanisinf):
        ift.myassert(isinstance(sample_list, ResidualSampleList))
        ift.myassert(sample_list.domain is hamiltonian.domain)

        if isinstance(sample_list.domain, ift.MultiDomain):
            if not (invariants is None or isinstance(invariants, ift.MultiField)):
                raise TypeError
        ...
        def _func(inp):
            ...
            tmp = tmp(ift.Linearization.make_var(inp))
    ...
    def apply_metric(self, x):
        def _func(inp):
            ...
            tmp = tmp(ift.Linearization.make_var(inp, want_metric=True))
            return tmp.metric(x)
