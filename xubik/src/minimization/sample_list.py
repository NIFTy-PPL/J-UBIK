...
class SampleListBase:
    ...
    def __init__(self, comm, domain, n_local_samples):
        ...
        ift.utilities.check_MPI_equality(self._domain, comm)
        ...
        self._n_samples = ift.utilities.allreduce_sum([self.n_local_samples], self.comm)
    ...
    @property
    def MPI_master(self):
        return ift.utilities.get_MPI_params_from_comm(self.comm)[2]
    ...
    def save_to_fits(self, file_name_base, op=None, samples=False, mean=False, std=False,
        ...
        obs_type : string or None
            ...
            is considered as a single frequency observation.
        """
        ...
        if mean:
            if obs_type is None:
                ...
                if obs_type == "RGB":
                    m = xu.energy_binning(m, energy_bins=3)
                xu.save_rgb_image_to_fits(m, file_name_base + "_mean", overwrite, self.MPI_master)
        if std:
            if obs_type is None:
                self._save_fits_2d(s, file_name_base + "_std.fits", overwrite)
            else:
                if obs_type == "RGB":
                    s = xu.energy_binning(s, energy_bins=3)
                xu.save_rgb_image_to_fits(s, file_name_base + "_std", overwrite, self.MPI_master)
        if samples:
            for ii, ss in enumerate(self.iterator(op)):
                if obs_type is None:
                    ...
                    if obs_type == "RGB":
                        ss = xu.energy_binning(ss, energy_bins=3)
                    xu.save_rgb_image_to_fits(ss, file_name_base + f"_sample_{ii}", overwrite, self.MPI_master)
    ...
    def average(self, op=None):
        ...
        n = self.n_samples
        return ift.utilities.allreduce_sum(res, self.comm) / n
    ...
    def _average_tuple(self, op):
        ...
        if len(self.local_indices) > 0:
            ...
            res = tuple(ift.utilities.allreduce_sum(rr, self._active_comm) / n for rr in res)
    ...
    def _list_local_sample_files(cls, file_name_base, comm=None):
        ...
        local_indices = range(*ift.shareRange(n_samples, ntask, rank))
...
class ResidualSampleList(SampleListBase):
    def __init__(self, mean, residuals, neg, comm=None):
        ...
        if any(elem == 0 for elem in ift.utilities.allreduce_sum([[len(self._r)]], comm)):
        ...
        if len(self._r) > 0:
            ...
            if isinstance(r_dom, ift.MultiDomain):
                try:
    ...
    def at(self, mean):
        ...
        """
        if isinstance(self._m, ift.MultiField) and self.domain is not mean.domain:
            mean = ift.MultiField.union([self._m, mean])
...
class SampleList(SampleListBase):
    def __init__(self, samples, comm=None, domain=None):
        ...
        self._s = samples
        for ss in self._s:
            ift.utilities.check_object_identity(ss.domain, self.domain)
...
def _none_to_id(obj):
    ...
    if obj is None:
        return lambda x: x
    if isinstance(obj, ift.Operator):
...
def _field2hdf5(file_handle, obj, name):
    if not isinstance(name, str):
        raise TypeError
    if isinstance(obj, ift.MultiField):
