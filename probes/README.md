...
| probe | pins |
| --- | --- |
| `p1_metadata_vs_response.py` | the two-quirk structure: `WcsAstropy` metadata pairs `shape[0]` with the FITS RA axis, while the response indexing pairs array dim 0 with the Dec coordinate |
...
| `p4_radio_adapter.py` | the radio response COMPLIES with the canonical frame via the explicit `canonical_sky_to_visibilities` adapter (canonical sky in, physical visibilities out) wrapping the raw backends |
...
| `p7_radio_roundtrip.py` | a CASA-minted radio observation roundtrips to a dirty image matching the canonical truth (external sign-anchor pin): the glyph survives the resolve response + gridder path with a dihedral `identity` verdict |
