...
WHAT THIS PROBES
    `WcsAstropy` takes public `shape=(nx, ny)` and
    `fov=(fov_x, fov_y)`, while numerical fields use `(ny, nx)`.
    Consequences pinned here, all on a RECTANGULAR grid:

    A. header pairing: FITS axis 1 (RA, CDELT1 < 0) is sized by
       public x; axis 2 (Dec) by public y.
    B. pixel scales are exposed in explicit XY and YX order.
    C. response coherence: the YX index coordinates of the full world
    ...
    D. `extent()` returns the house-recipe imshow 4-tuple
       (left, right, bottom, top) = (+hx, -hx, -hy, +hy).
...
def main() -> None:
    ...
    n_dec, n_ra = 6, 10
    wcs = WcsAstropy(center=center, shape=(n_ra, n_dec),
                     fov=(10 * u.arcsec, 6 * u.arcsec))

    # --- A: header pairing --------------------------------------------
    header = wcs.to_header()
    assert header["CRPIX1"] == n_ra / 2 + 0.5, "RA axis not sized by public x"
    assert header["CRPIX2"] == n_dec / 2 + 0.5, "Dec axis not sized by public y"
    ...
    print("A  header: axis1(RA) <- public x, axis2(Dec) <- public y")

    # --- B: distances index-matched ------------------------------------
    assert wcs.shape_xy == (n_ra, n_dec)
    assert wcs.shape_yx == (n_dec, n_ra)
    d = wcs.pixel_scales_yx.to(u.arcsec).value
    assert np.allclose(d, [6 / n_dec, 10 / n_ra]), \
        f"distances not index-matched to array dims: {d}"
    print("B  explicit XY/YX pixel scales describe public and array axes")
    ...
    assert np.allclose(ext, (5.0, -5.0, -3.0, 3.0)), ext
    print("D  extent() = (+hx, -hx, -hy, +hy) — North-up/East-left")
