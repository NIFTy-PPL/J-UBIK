...
def test_initialization(sample_obs_info):
    ...
    assert chandra_obs.obsInfo["event_file"] == join(
        "chandra_test_data/", "primary/structure_only_evt.fits"
    )
    assert chandra_obs.obsInfo["aspect_sol"] == join(
        "chandra_test_data/", "primary/structure_only_asol.fits"
    )
    assert chandra_obs.obsInfo["bpix_file"] == join(
        "chandra_test_data/", "primary/structure_only_bpix.fits"
    )
    assert chandra_obs.obsInfo["mask_file"] == join(
        "chandra_test_data/", "secondary/structure_only_msk.fits"
    )
