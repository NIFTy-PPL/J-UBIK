...
@pmp("di,dj", ((3, 5), (-4, 2)))
def test_response_phase_convention_uses_east_north_offsets(di, dj):
    ...
    expected = np.broadcast_to(
        np.exp(2j * np.pi * (uu * x_east + vv * y_north))[None], vis_ducc.shape
    )
    ...
    wrong = np.exp(2j * np.pi * (uu * x_east - vv * y_north))[None]
