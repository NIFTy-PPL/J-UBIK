"""Orientation glyph for the roundtrip tests: a letter "F" on the sky.
...
- `rasterize_canonical` paints it in the canonical frame (dim0 = +Dec,
  dim1 = -RA; docs/source/user/canonical-sky-design.md), where it must
  come out with the stroke along +dim0 and the arms along +dim1.  Under
  the house plot (origin="lower", East left) it reads as a normal F.
- `jwst_fixture` and `radio_fixture` paint the same `sample_points`
  through their data WCS, anchoring the truth to absolute sky positions.
...
Self-test:  uv run python test/conventions/glyph.py
"""
