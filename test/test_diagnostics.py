...
def sample_data():
    ...
    op = lambda x: x
    response_dict = {"exposure": lambda x: x, "R": lambda x: x, "mask_adj": lambda x: x}
    return sky, op, sky, data, response_dict


def test_calculate_uwr(sample_data):
    pos, op, ground_truth, _, response_dict = sample_data
    res, exposure_mask = ju.calculate_uwr(
        pos, op, ground_truth, response_dict, abs=True, exposure_mask=True, log=True
    )
