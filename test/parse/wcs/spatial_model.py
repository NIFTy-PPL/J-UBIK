...
def test_spatial_model_from_yaml_dict():
    ...
    assert spatial_model.wcs_model.center == SkyCoord(
        ra=scc['ra'], dec=scc['dec'])
    assert spatial_model.shape == (grid_config['sdim'],)*2
    assert spatial_model.fov == (u.Quantity(grid_config['fov']), )*2
    assert spatial_model.wcs_model.rotation == u.Quantity(
        grid_config['rotation'])
...
def test_spatial_model_from_config_parser():
    ...
    # Test shape
    assert spatial_model.shape == (grid_config['space npix x'],
                                   grid_config['space npix y'])

    # Test fov
    fov = (resolve_str_to_quantity(grid_config['space fov x']),
           resolve_str_to_quantity(grid_config['space fov y']))
    assert spatial_model.fov == fov
