...
from jubik0.minimization_parser import (
    ...
    n_samples_factory,
    sample_mode_factory,
    linear_sample_kwargs_factory,
    nonlinearly_update_kwargs_factory,
    kl_kwargs_factory,
    MinimizationParser,
)
...
config = {
    "delta": {"switches": [0, 5], "values": [1.0e-5, 1.0e-6]},
    "samples": {
        "switches": [0, 10],
        "n_samples": [4, 5],
    ...
    "kl_minimization": {"switches": [0, 10], "kl_maxiter": [10, 13]},
...
class TestMinimizationParser:
    ...
    @pytest.mark.parametrize(
        "mini_cfg, iteration, total_iterations, expected",
        [
    ...
    @pytest.mark.parametrize(
        "type, config, switches_index, expected",
        [
    ...
    def test_delta_logic(self, type, config, switches_index, expected):
        assert _delta_logic(type, config, None, 0, switches_index, 10) == expected
    ...
    def test_linear_sample_kwargs_factory(self):
        ...
        kwargs = lin_kwargs(0)
        assert kwargs["cg_kwargs"]["absdelta"] == 1.0e-5
    ...
    def test_nonlinearly_update_kwargs_factory(self):
        ...
        assert kwargs["minimize_kwargs"]["xtol"] == 1.0e-6
    ...
    def test_kl_kwargs_factory(self):
        ...
        assert kwargs["minimize_kwargs"]["absdelta"] == 1.0e-4
    ...
    def test_minimization_parser(self):
        ...
        assert parser.nonlinearly_update_kwargs(7)["minimize_kwargs"]["xtol"] == 1.0e-6
