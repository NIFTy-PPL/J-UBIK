...
npix_e = 4
fov = 4.0
elim = (2.0, 10.0)  # energy range in keV #FIXME check limits
...
zp_position_space = ift.RGSpace(
    [npix_s_zp, npix_s_zp], distances=[2.0 * fov / npix_s]
)  # FIXME less zeropadding enough?
...
ic_newton = ift.AbsDeltaEnergyController(
    name="Newton", deltaE=0.5, convergence_level=3, iteration_limit=4
)
...
if False:
    ...
    for ii in range(10):
        if ii >= 3:
            ic_newton = ift.AbsDeltaEnergyController(
                name="Newton", deltaE=0.5, iteration_limit=5, convergence_level=3
            )
            minimizer_sampling = ift.NewtonCG(
                ift.AbsDeltaEnergyController(
