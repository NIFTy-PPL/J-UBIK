...
# Isolated numbers overcount what a sub-model costs inside the fused
# program, and sub-models usually nest, so compare rows against the root
# rather than adding them up. The numbers are meant for *relative*
# comparison between sub-models and for tracking regressions, not as
...
#
# It is CPU-friendly and needs no data downloads. Flops estimates may be
# missing on the CPU backend or older jax versions.
...
# `root`. Here `sky` wraps `diffuse` and `response` wraps `sky`, so each
# row already contains the ones above it; the last row is what
# `optimize_kl` actually jits.
...
# device memory in use, its process-wide peak, and the number of live
# device arrays, one JSON line each plus a log line. Memory growth over
# iterations (e.g. compilation-cache bloat on sample-mode switches)
# becomes visible instead of guessed; pair it with
# `ju.clear_jax_compilation_cache` to verify the clearing actually helps.
# The CPU backend reports no memory statistics, so those two fields are
# `-` there; the live-array count still works.
