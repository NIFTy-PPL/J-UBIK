...
def plot(array, pixel_measure=None, output_file=None, logscale=False, title=None, colorbar=True,
         figsize=(8, 8), dpi=200, n_rows=None,
         share_x=True, share_y=True, fs=14, alpha=0.8,
             bbox_info= [(7, 4), 7,  30], **kwargs):
    ...
    for i in range(array.shape[0]):
        ...
        if pixel_measure is not None:
            ...
            x0, y0 = 10, 10
            x1, y1 = pixel_measure+10, 10

            x2, y2 = 10, 8
            x3, y3 = 10, 12

            x4, y4 = pixel_measure+10, 8
            x5, y5 = pixel_measure+10, 12
            rect = patches.Rectangle(bbox_info[0], pixel_measure+bbox_info[1],
                                 bbox_info[2], facecolor='black', alpha=alpha)
            ax[i].add_patch(rect)
            ax[i].text(int(pixel_measure/2)+7, 14, f"{distance_measure}'", fontsize=fs,
                       color='white')
...
def plot_rgb(x, sat_min=[0, 0, 0], sat_max=[1, 1, 1], sigma=None, log=False,
             title=None, pixel_measure=None, fs=14, figsize=(8, 8), dpi=200,
    ...
    if pixel_measure is not None:
        ...
        rect = patches.Rectangle(bbox_info[0], pixel_measure+bbox_info[1],
                                 bbox_info[2],  facecolor='black', alpha=alpha)
        ax.add_patch(rect)
        ax.text(int(pixel_measure/2)+7, 14, f"{distance_measure}'", fontsize=fs,
                   color='white')
