...
# are complete:
#
# 1. Install [JWST](https://jwst-pipeline.readthedocs.io/en/latest/getting_started/install.html)
# 2. Install [stpsf](https://stpsf.readthedocs.io/en/latest/installation.html).
...
#
# In the `JWST_demo.yaml` config file, you can define several parameters. A sample config file can be found in 'demos/configs'. Here a [link to the file](https://github.com/NIFTy-PPL/J-UBIK/blob/main/demos/configs/jwst_demo.yaml).
#  
...
parser.add_argument(
    ...
    nargs="?",
    const=1,
    default="configs/jwst_demo.yaml",
)
...
# %% [markdown]
# ## Sky Model
# To configure the Sky Model add the following parameters to your config YAML.
#
...
for fltname in filters.keys():
    ...
    key, subkey = random.split(key)
    data = response(mock_sky) + random.normal(subkey, response.target.shape) * noise_std
    all_filter_data.append(data)

    likelihood = build_gaussian_likelihood(data, noise_std)
    likelihood = likelihood.amend(response, domain=jft.Vector(response.domain))
...
ju.plot_result(all_filter_data, output_file=join(file_info["res_dir"], "data.png"))
